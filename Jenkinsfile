pipeline {
    ...
    stages {
        stage('Preparar Entorno') {
            steps {
                ...
                bat 'C:\\Users\\andre\\python3.13\\python.exe -m pip install --upgrade pip && pip install -r requirements.txt'
            }
        }
    ...
    post {
        ...
        success {
            echo '### :heavy_check_mark: Todos los tests pasaron correctamente ✅'
        }
        failure {
            echo '### :x: La ejecución del pipeline falló ❌. Enviando correo electrónico...'
